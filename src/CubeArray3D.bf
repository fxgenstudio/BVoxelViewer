...
namespace VoxelViewer
{
	...
	public class CubeArray3D
	{
		...
		public static SmallCube m_emptyCube;
			...
			for (x = 0; x < CHUNKSIZEX; x++)
			{
				for (z = 0; z < CHUNKSIZEZ; z++)
				{
					for (y = 0; y < CHUNKSIZEY; y++)
					{
						...
						delete m_arrayChunks[offset];
						m_arrayChunks[offset] = null;
					}
				}
			}
			delete m_arrayChunks;
			m_arrayChunks= null;
		}
		...
		public bool GetNeighbourCube(int _x, int _y, int _z, CubeHelpers.SIDE side, out SmallCube _c)
		{
			_c = m_emptyCube;
			int x, y, z;
			x = _x;
