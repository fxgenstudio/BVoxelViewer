...
namespace VoxelViewer
{
		...
		public class MagicaVoxelLoader
		{
			...
			/// From a file name
			/// </summary>
			public bool ReadFile(String _strFileName)
			{
				bool bret = false;

				//Load file into a memory stream for faster vox loading
				var stream = new FileStream();
				var result = stream.Open(_strFileName, FileAccess.Read);
				if (result case .Ok)
				{
					...
					//Read vox from memory stream
					bret = ReadFile(mstream);
		    ...
		    public bool ReadFile(Stream _stream)
		    {
		        ...
		        while (_stream.Position < _stream.Length)
		        {
					...
					String str = scope String((char8*)&chunkName, 4);
					Debug.WriteLine(str);

					if (chunkName==0x00000000)
						break;
#unwarn
...
#unwarn
					...
					{
						_stream.Seek(_stream.Position+ chunkSize);
					}
		        }

				//Set default palette if not found into file
				if (bPaletteChunk==false)
				{
					for (int i = 0; i < 256; i++)
					{
						m_palette[i] =  Color(default_palette[i]);
					}
				}
			...
			static uint32 []default_palette = new  uint32[] (
